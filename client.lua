...
AddEventHandler('SaltyChat_TalkStateChanged', function(SaltyisTalking)
            if voice.current == 0 then
                SendNUIMessage({action = "setVoiceLevel", level = 2}); -- coded by TutoHacks
            elseif voice.current == 1 then
                SendNUIMessage({action = "setVoiceLevel", level = 3}); -- coded by TutoHacks
            elseif voice.current == 2 then
                SendNUIMessage({action = "setVoiceLevel", level = 1}); -- coded by TutoHacks
            end
end)
    ...
    while true do
		...
		if IsControlJustPressed(1, 243) then
            ...
            if voice.current == 0 then
				...
				TriggerEvent("notification", "#3333FF", "Voice System", "Normal (6 meters)") -- coded by TutoHacks
            elseif voice.current == 1 then
                NetworkSetTalkerProximity(voice.shout)
                SendNUIMessage({action = "setVoiceLevel", level = 3});
				voiceCoords = voice.shout
				TriggerEvent("notification", "#ab0503", "Voice System", "Shout (15 meters)") -- coded by TutoHacks
            elseif voice.current == 2 then
                NetworkSetTalkerProximity(voice.whisper)
                SendNUIMessage({action = "setVoiceLevel", level = 1});
				voiceCoords = voice.whisper
				TriggerEvent("notification", "#E4E652", "Voice System", "Whisper (2 meters)") -- coded by TutoHacks
            end
...
AddEventHandler('esx:activateMoney', function(e)
	SendNUIMessage({action = "setMoney", money = e}) -- coded by TutoHacks
end)
...
if  data == nil or data == '' then
	SendNUIMessage({action = "hide"}) -- coded by TutoHacks

else
	SendNUIMessage({action = "show"}) -- coded by TutoHacks
